...
namespace SimpleVisualTheme.Theme.Data.Internal
{
    internal class ThemeData : IThemeData
    {
        ...
        /// <summary></summary>
        public bool AddProperty(string key) => _properties.TryAdd(key, new ThemeDataProperty());

        /// <summary></summary>
        public bool AddProperty(string key, string type, string value, string xamlKey) =>
            _properties.TryAdd(key, new ThemeDataProperty() { Type = type, Value = value, XamlKey = xamlKey });

        /// <summary></summary>
        public IThemeDataProperty? GetProperty(string key) => _properties.TryGetValue(key, out var value) ? value : null;

        /// <summary></summary>
        public IThemeDataProperty GetPropertyNullThrow(string key) =>
            _properties.TryGetValue(key, out var value) ? value : throw new InvalidDataException($"Property '{key}' is null.");

        /// <summary></summary>
        public ICollection<string> PropertyKeys() => new List<string>(_properties.Keys);

        /// <summary></summary>
        public bool RemoveProperty(string name) => _properties.TryRemove(name, out _);
